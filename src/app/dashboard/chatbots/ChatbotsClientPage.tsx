...
export default function ChatbotManagerUI() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (demoUserStr) {
        ...
        if (localDataStr) {
          setBots(JSON.parse(localDataStr));
        } else {
          setBots([]);
        }
        setLoading(false);
      ...
      if (currentChatbot) {
        ...
        if (isSandbox) {
          ...
          if (res.success && res.chatbot) {
            setBots(bots.map(b => b.id === currentChatbot.id ? { ...b, ...botPayload } : b));
            setShowFormModal(false);
          } else {
  ...
  const handleDeleteChatbot = async (id: string) => {
    ...
    if (isSandbox) {
      const updatedList = bots.filter((b) => b.id !== id);
      localStorage.setItem('uipro_sandbox_chatbots', JSON.stringify(updatedList));
      setBots(updatedList);
    } else {
      let clientUserId = userId;
